...
cIntl = 30.328921 # intercept for international stock returns
volIntl = 1.818674 # volatility coefficient for international stock returns
durIntl = 6.297304 # duration coefficient for international stock returns
...
def output(initialW, initialFlow, growthFlow, timeHorizon, bondShare0, bondShare1, intlShare):
    ...
    pathData, paths = simWealth(currVol, currRate, initialW, initialFlow, growthFlow, timeHorizon, bondShare0, bondShare1, intlShare)
    ...
    for display in range(NDISPLAYS):
        ...
        if (endWealth == 0): # this path ended with zero wealth
            pathLabel = '0' + rankText + ' Gone Bust !!!' + '\nIn ' + str(int(pathData[index])) + ' Years'
    ...
    # plt.twinx()
    # allocation = np.linspace(bondShare0, bondShare1, timeHorizon)
    # plt.plot(np.array(range(1, timeHorizon + 1)), allocation, linestyle = ':', label = 'Bond Share in Portfolio')
    # plt.gca().set_ylabel('Bond Share')
