...
lrate = np.log(rate)
USReturns = np.log(price[1:] + dividend) - np.log(price[:-1]) # arithmetic returns
...
normIntlReturns = 100 * np.log(1 + intlReturns)/vol[-NINTL:] # normalized intl returns in %
bonds = DF['bonds']
wealthBond = bonds['Bond Wealth'].values
NBOND = len(wealthBond) - 1
bondRet = 100 * np.diff(np.log(wealthBond)) # bond returns in %
