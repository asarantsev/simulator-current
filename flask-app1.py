...
USReturns = np.log(price[1:] + dividend) - np.log(price[:-1])
...
intlReturns = intl['International'].values
NINTL = len(intlReturns) # length of data for international returns
...
def simulation(initialVol, nYears, intlStocks):
    ...
    fittedCoeffs, simResiduals = fitData(nYears)
