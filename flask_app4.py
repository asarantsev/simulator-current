...
currVol = 7.98 # S&P 500 Volatility of 2024
currRate = 5.80 # BAA rate December 2024 average
...
def simReturns(initVol, initRate, nYears, bondP, intlP):
    ...
    simOverall = simBond * bondP + simStock * (1 - bondP) # simulate combined stock and bond portfolio
...
def simWealth(initVol, initRate, initialW, initialFlow, growthFlow, nYears, bondShare, intlShare):

    #simulate returns of this portfolio
    simRet = simReturns(initVol, initRate, nYears, bondShare, intlShare)
...
def setupText(initialWealth, initialFlow, growthFlow, timeHorizon, bondShare, intlShare):
    ...
    bondPortfolioText = 'Portfolio: ' + percent(1 - bondShare) + ' Stocks\n' + percent(bondShare) + ' Bonds: US Investment-Grade'
    portfolioText = bondPortfolioText + '\n' + stockPortfolioText
...
def output(initialW, initialFlow, growthFlow, timeHorizon, bondShare, intlShare):

    # simulate wealth process of a portfolio
    timeAvgRet, paths = simWealth(currVol, currRate, initialW, initialFlow, growthFlow, timeHorizon, bondShare, intlShare)
    ...
    SetupText = setupText(initialW, initialFlow, growthFlow, timeHorizon, bondShare, intlShare)
...
def compute():
    ...
    # share of bonds in portfolio, converted from %
    bond = float(request.form['bond'])*0.01
    ...
    output(initialWealth, initialFlow, growthFlow, nYears, bond, intl)
